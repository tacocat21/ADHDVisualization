...
def _conv_layer(input_channels, output_channels, kernel_size, padding=1):
    return torch.nn.Sequential(nn.Conv3d(input_channels, output_channels, kernel_size=kernel_size, padding=padding),
                               nn.ReLU())
...
class StructuralModel3D(nn.modules.Module):
    def __init__(self):
        ...
        self.conv1 = _conv_layer(1, 32, kernel_size=5, padding=1)
        self.max_pool_2 = torch.nn.MaxPool3d(max_pool_shape)
        self.conv2 = _conv_layer(32, 32, kernel_size=(3, 5, 5), padding=1)
        self.conv3 = _conv_layer(32, 64, kernel_size=3)
        self.conv4 = _conv_layer(64, 64, kernel_size=3)
        self.conv5 = _conv_layer(64, 32, kernel_size=3)
